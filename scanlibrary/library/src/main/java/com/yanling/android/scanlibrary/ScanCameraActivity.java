...
public class ScanCameraActivity extends Activity {
    ...
    private PreviewCallback previewCallback = new PreviewCallback() {
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            ...
            //将相机的数据翻转90
            byte[] buff = onTurn(data, size.width, size.height);
            //解析数据(注意要进行翻转)
            String result = ScanUtils.decodeZxing(buff, size.height, size.width);
            //返回扫描结果
            if (result != null){
                //封装扫码结果
     ...
     * @return
     */
    private byte[] onTurn(byte[] data, int width, int height){
        // 这里需要将获取的data翻转一下，因为相机默认拿的的横屏的数据
        byte[] buff = new byte[data.length];
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                buff[x * height + height - y - 1] = data[x + y * width];
            }
        }
        return buff;
    }
