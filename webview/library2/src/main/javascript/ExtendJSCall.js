...
var extendJSCall = (function(){
    ...
    //定义Native端回调JS对象名字空间
    var const_jsCallback = "extendJSCallbacks";
    //定义成功和失败回调对象名字空间
    var const_callbackSuccess = "callbackSuccess";
    var const_callbackError = "callbackError";
    // 初始化全局回调callbacks对象，相当于map全局保存Native回调js对象
    window[const_jsCallback] = new Object;
    var callbacks = window[const_jsCallback];
   ...
   /**
     ...
     */
    var packageApiData = function(apiKey, action, data, callback, isAsync){
        var jsonObj = {
            "action": action,
            "data": data,
            "callback": callback,
        ...
        //拼接返回值协议
        var result = "jscall://" + apiKey + "?" + JSON.stringify(jsonObj);
        //对url协议数据进行转码返回
     ...
     */
    var jsCall = function(jsToNativeMode, urlData){
    ...
    //定义常量标识JSCall
    extendJSCall.const_jsToNativeMode = const_jsToNativeMode;
    ...
    extendJSCall.enqueue = function(apiKey, action, data, success, error){
        ...
        callbacks[callbackId] = {
            callbackSuccess: function(data, isKeep){
                //回调成功结果
                success(data);
                //判断是否保持住回调
                if (isKeep == "false"){
